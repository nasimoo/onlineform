...
export async function POST(req: NextRequest) {
  try {
    ...
    // Optional server-side sampling to reduce write volume
    const sampling = Number(process.env.EVENTS_SAMPLING || '1'); // 0..1
    if (Math.random() <= Math.max(0, Math.min(1, sampling))) {
...
async function appendWithRetry(
  ...
  let attempt = 0;
  let delayMs = 400;
  // Use Events tab; ensure it exists in the sheet
  ...
  throw new Error('Exceeded retry attempts for events append');
}
